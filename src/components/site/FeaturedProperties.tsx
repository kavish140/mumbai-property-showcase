        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {featured.map((p) => (
            <Link key={p.id} to="/properties/$id" params={{ id: p.id }} className="block">
              <Card className="group h-full overflow-hidden border-border bg-card pt-0 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img
                    ...
                    loading="lazy"
                  />
                  <Badge className={`absolute left-3 top-3 border ${statusVariant[p.status]}`}>
                    {p.status}
                  </Badge>
